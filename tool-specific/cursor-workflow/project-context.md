...
> **Purpose:** Persistent project context for Cursor. For requirements see [`requirements-analysis.md`](../../requirements-analysis.md).
...
- All Core acceptance criteria in `requirements-analysis.md` §14 are met
...
Every feature should trace back to a requirement ID in `requirements-analysis.md` and forward to a test.
...
│   ├── spec.md
│   └── README.md
├── requirements-analysis.md   # Root assessment artifacts
├── design-notes.md
├── debugging-notes.md
...
| **Docs are first-class** | Root assessment artifacts are not an afterthought — reviewers read them |
...
1. **Provide context** — Reference this file, `requirements-analysis.md`, and `implementation-plan.md`
...
- Overwrite `ai-prompts/*.md` — **append only**
...
After every major task, append to [`ai-prompts/`](../../ai-prompts/) (see [`tool-workflow.md`](../../tool-workflow.md)).
...
Document failures and fixes in `debugging-notes.md` and `test-results.md`.
...
| **`ai-prompts/`** | Append-only AI interaction log by activity | After every major AI task |
| **`debugging-notes.md`** | Debug and test evidence | When bugs found/fixed |
| **`test-results.md`** | Test run outcomes | After test runs |
...
6. Run tests
7. Brief architecture overview and link to `design-notes.md`
...
Keep these in `tool-specific/cursor-workflow/`:
